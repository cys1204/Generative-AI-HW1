...
def build_dataloader(config: TrainingConfig) -> DataLoader:
    ...
    dataset = UnlabeledImageDataset(config.data_root, transform=transform)
    print(f"Loaded {len(dataset)} images from {config.data_root}")
    return DataLoader(
        dataset,
...
def parse_args() -> TrainingConfig:
    ...
    parser.add_argument(
        "data_root",
        type=Path,
        help="Path to the CelebA image directory (e.g., data/celeba/img_align_celeba).",
    )
