...
def build_dataloader(config: SRTrainingConfig) -> DataLoader:
    ...
    print(f"Loaded {len(dataset)} CIFAR-10 images from {config.data_root}")
    return DataLoader(
        dataset,
...
def train(config: SRTrainingConfig) -> None:
    ...
    for epoch in range(1, config.epochs + 1):
        for step_in_epoch, (real_hr, _) in enumerate(dataloader, start=1):
            ...
            if (
                ...
                and step_in_epoch >= config.max_steps_per_epoch
            ):
                break
...
def parse_args() -> SRTrainingConfig:
    ...
    parser.add_argument("--max-steps-per-epoch", type=int, default=None, help="Limit number of batches per epoch for quick runs.")
